...
exports.protect = catchAsync(async (req, res, next) => {
  ...
  if (!token) {
    return next(
      new AppError(' You are not log in Pleaser log in to get access', 401)
    );
  }
  ...
  if (!currentUser) {
    return next(
      new AppError('The user belongimg to this token does no longer exist', 401)
    );
  }

  // 4) check if changed password after token was issued
  ...
  // Grant Access to protected code
